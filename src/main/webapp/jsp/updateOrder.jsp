...
<head>
    ...
    <script>
        ...
        function Cout(params,status) {
//            重要操作会有提示
            var r=confirm("你确定要执行该操作吗？")
            if (r==true)
            {
                self.location.href="<%=request.getContextPath()%>/order/updateOrder?str="+params+"&status="+status+"&pageIndex="+${pageIndex};
            }
        }
    </script>
</head>
            ...
            <div class="row">
                <div class="panel panel-default">
                    ...
                    <div class="bootstrap-admin-panel-content">
                        <table class="table">
                            ...
                            <c:forEach items="${pageInfo}" var="s">
                                <tr>
                                    ...
                                    <td>${s.price}</td>
                                    <td>${s.orderCreateTime.substring(0,10)}</td>
                                    <td>
                                        <button class="btn btn-primary btn-sm" id="cout" onclick="Cout(${s.id},2)">
                                            退房
                                        </button>
                                        <button class="btn btn-danger btn-sm" id="canc" onclick="Cout(${s.id},0)">
                                            取消订单
                                        </button>
                                    </td>
                                </tr>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <li>当前第${pageIndex.toString()}页，总共${pages.toString()}页，总共有${allCounts.toString()}条订单待完成</li>
                            </div>
                            <div class="col-md-6">
                                <div class="pagination-container pagination-container-first">
                                    <ul class="pagination">
                                        <c:if test="${allCounts>0}">
                                            <li><a href="<%=request.getContextPath()%>/order/updateOrder?pageIndex=1">首页</a></li>
                                        </c:if>
                                        <c:if test="${pageIndex>1}">
                                            <li><a href="<%=request.getContextPath()%>/order/updateOrder?pageIndex=${pageIndex-1}">上一页</a></li>
                                        </c:if>
                                        <c:forEach begin="1" end="${Pages}" varStatus="loop">
                                            <c:set var="active" value="${loop.index==pageIndex?'active':''}"/>
                                            <li class="${active}"><a
                                                    href="<%=request.getContextPath()%>/order/updateOrder?pageIndex=${loop.index}"> ${loop.index}</a>
                                            </li>
                                        </c:forEach>
                                        ...
                                        <c:if test="${pageIndex<pages}">
                                            <li>
                                                <a href="<%=request.getContextPath()%>/order/updateOrder?pageIndex=${pageIndex+1}">下一页</a>
                                            </li>
                                        </c:if>
                                        <c:if test="${allCounts>0}">
                                            <li><a href="<%=request.getContextPath()%>/order/updateOrder?pageIndex=${pages}">末页</a></li>
                                        </c:if>
                                    </ul>
                                </div>
                            </div>
